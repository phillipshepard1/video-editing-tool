...
export default function DashboardPage() {
  ...
  const autoSaveSession = async (data: any) => {
    try {
      ...
      if (result.success) {
        console.log('Session auto-saved:', result.sessionId);
  ...
  const handleSessionLoad = (loadedSession: any) => {
    console.log('Loading session:', loadedSession);
    
    // Set video URL if available
    if (loadedSession.supabaseUrl) {
      setVideoUrl(loadedSession.supabaseUrl);
    }
    
    // Fix video duration if it's 0 by calculating from segments
    let actualVideoDuration = loadedSession.videoDuration || 0;
    if (actualVideoDuration === 0 && loadedSession.segments && loadedSession.segments.length > 0) {
      // Try to infer duration from segments
      const maxTime = Math.max(...loadedSession.segments.map((seg: any) => {
        const endTime = typeof seg.endTime === 'string' ? parseTimeStringToSeconds(seg.endTime) : seg.endTime;
        return endTime || 0;
      }));
      // If max time is reasonable (less than 2 hours), use it
      if (maxTime > 0 && maxTime < 7200) {
        actualVideoDuration = maxTime;
      } else {
        // Default to 12 minutes if we can't determine
        actualVideoDuration = 720; // 12 minutes in seconds
      }
    }
    ...
    // Set analysis data and fix corrupt timestamps
    const segments = loadedSession.segments || [];
    const enhancedSegments = segments.map((seg: any, index: number) => {
      // Fix timestamps - keep as strings but fix format
      let startTime = seg.startTime;
      let endTime = seg.endTime;
      let duration = seg.duration;
      
      // Parse and validate times
      let startSeconds = typeof startTime === 'string' ? 
        parseTimeStringToSeconds(startTime) : startTime;
      let endSeconds = typeof endTime === 'string' ? 
        parseTimeStringToSeconds(endTime) : endTime;
      
      // Validate and fix unreasonable times
      if (startSeconds > actualVideoDuration || startSeconds < 0) {
        startSeconds = Math.min(Math.max(0, startSeconds), actualVideoDuration);
      }
      if (endSeconds > actualVideoDuration || endSeconds < 0) {
        endSeconds = Math.min(Math.max(0, endSeconds), actualVideoDuration);
      }
      ...
      // Format back to strings in correct format
      startTime = formatSecondsToTimeString(startSeconds);
      endTime = formatSecondsToTimeString(endSeconds);
    ...
    setAnalysis({
      ...
      summary: {
        originalDuration: loadedSession.originalDuration || actualVideoDuration,
        finalDuration: loadedSession.finalDuration || 0,
        timeRemoved: loadedSession.timeSaved || 0,
        segmentCount: loadedSession.segmentsToRemove || segments.length
      },
      supabaseUrl: loadedSession.supabaseUrl
    });
    
    // Set enhanced analysis if available
    if (loadedSession.clusters) {
      // Fix cluster timestamps too
      const fixedClusters = loadedSession.clusters.map((cluster: any) => ({
        ...cluster,
        attempts: cluster.attempts?.map((attempt: any) => ({
          ...attempt,
          startTime: typeof attempt.startTime === 'string' ? 
            formatSecondsToTimeString(Math.min(parseTimeStringToSeconds(attempt.startTime), actualVideoDuration)) : 
            attempt.startTime,
          endTime: typeof attempt.endTime === 'string' ? 
            formatSecondsToTimeString(Math.min(parseTimeStringToSeconds(attempt.endTime), actualVideoDuration)) : 
            attempt.endTime
        })) || [],
        winner: cluster.winner && {
          ...cluster.winner,
          startTime: typeof cluster.winner.startTime === 'string' ? 
            formatSecondsToTimeString(Math.min(parseTimeStringToSeconds(cluster.winner.startTime), actualVideoDuration)) : 
            cluster.winner.startTime,
          endTime: typeof cluster.winner.endTime === 'string' ? 
            formatSecondsToTimeString(Math.min(parseTimeStringToSeconds(cluster.winner.endTime), actualVideoDuration)) : 
            cluster.winner.endTime
        }
      }));
      
      setEnhancedAnalysis({
        segments: enhancedSegments,
        contentGroups: fixedClusters,
        summary: {
          originalDuration: loadedSession.originalDuration || actualVideoDuration,
          finalDuration: loadedSession.finalDuration || 0,
          timeRemoved: loadedSession.timeSaved || 0,
  ...
  const handleUploadAndAnalyze = async () => {
    ...
    try {
      ...
      // Auto-save session after successful analysis
      await autoSaveSession({
        videoUrl,
        supabaseUrl: uploadResult.supabaseUrl,
        videoFilename: file?.name || 'video.mp4',
        videoDuration,
        videoSizeMb: file ? file.size / (1024 * 1024) : null,
        geminiFileUri: uploadResult.fileUri,
        segments: enhancedSegments,
        clusters: enhancedResult?.contentGroups || null,
        clusterSelections: null,
        originalDuration: videoDuration,
        finalDuration: analysisData.summary.finalDuration,
        timeSaved: analysisData.summary.timeRemoved,
        totalSegments: enhancedSegments.length,
        segmentsToRemove: enhancedSegments.filter(s => s.selected).length
      });
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="mb-8">
            <SessionManager
              ...
              onSessionSave={(sessionId) => setCurrentSessionId(sessionId)}
            />
          </div>
