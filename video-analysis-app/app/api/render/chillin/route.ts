  ...
  try {
    ...
    try {
      ...
      if (!status || typeof status !== 'object') {
        console.error('Invalid status response from getRenderStatus:', status);
        return NextResponse.json({
          success: false,
          renderId,
          status: 'error',
          error: 'Invalid response from render service',
          details: 'The render service returned an unexpected response format'
        });
      }
      
      // Ensure status has required fields
      if (!status.status) {
        console.error('Status response missing status field:', status);
        return NextResponse.json({
          success: false,
          renderId,
          status: 'error',
          error: 'Invalid status response',
          details: 'The render service response is missing required fields'
        });
      }
